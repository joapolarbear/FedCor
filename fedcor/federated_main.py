...
def _run_with_one_seed(seed, args, device, gpr_device, file_name, start_time):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if args.gpr:
            if epoch >= args.gpr_begin:
                if epoch <= args.warmup:# warm-up
                    ...
                    print("Training GPR")
                    gpr.Train(lr=1e-2, llr=0.01, max_epoches=args.GPR_Epoch, 
                              schedule_lr=False, update_mean=args.update_mean, verbose=args.verbose)
        ...
        phase='Train'
        wandb_record[f'{phase}/Loss'] = sum(list_loss)/len(list_loss)
        wandb_record[f'{phase}/Acc'] = train_accuracy[-1]
        phase='Test'
        wandb_record[f'{phase}/Loss'] = test_loss
        wandb_record[f'{phase}/Acc'] = test_acc
        if AVAILABLE_WANDB:
